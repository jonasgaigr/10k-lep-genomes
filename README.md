...
`images/pipeline.svg` is drawn, not photographed. Redraw it with:

```bash
quarto run tools/make-pipeline-figure.ts
```

`quarto run` uses Quarto's own bundled Deno, so that needs nothing on PATH
beyond Quarto itself. The script derives the figure's canvas from the same
layout ratios as `aopk.scss`, so it lands exactly on the theme's content width
and `.no-leaf` safe height, and it refuses to write a figure whose labels no
longer fit their boxes.
...
tools/make-pipeline-figure.ts
                        redraws images/pipeline.svg (`quarto run`)
template.qmd            starter deck showing every slide type
images/priklad.svg      placeholder picture — replace it
images/pipeline.svg     GENERATED: sample-to-indicator pipeline, banded by
